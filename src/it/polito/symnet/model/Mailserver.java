...
public class Mailserver {
	...
	public Mailserver(String address, List<String> n, Map<String, String> addresses, String name, Map<String, String> protocols) {
		...
		ms.add(0,new StringBuilder().append("dst host ").append(address).toString());
		ms.add(1, "-");
		declaration.put("ms", ms);
	...
	public List<String> generateDeclaration() {
		...
		for(Entry<String, List<String>> decl : declaration.entrySet()){		
			...
			}else if(decl.getKey().compareTo("ms")==0){
	...
	public List<String> generateLink(Network n) {
		...
		//StringBuilder tmp=new StringBuilder().append("ms[0]").append(symbol).append("router");
	
		StringBuilder tmp=new StringBuilder().append("ms[0]").append(symbol).append("response");
		links.add(tmp.toString());
	
		StringBuilder tmp2=new StringBuilder().append("ms[1]").append(symbol).append("Discard");
