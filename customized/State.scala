...
object State {
 ...
 def bigBang: State = {
   ...
   val afterBigBang = InstructionBlock (
     ...
     CreateTag("L7", L4Tag + 160),
   
     Allocate(ApplicationProto, 8),
     Assign(ApplicationProto, SymbolicValue()),

     
     Allocate(Body, 8),
     Assign(Body, SymbolicValue()),
     Constrain(Body, :&:(:>=:(ConstantValue(0)), :<=:(ConstantValue(65536)))),

     Allocate(EmailFrom, 16),
     Assign(EmailFrom, SymbolicValue()),
     Constrain(EmailFrom, :&:(:>=:(ConstantValue(0)), :<=:(ConstantValue(65536)))),
